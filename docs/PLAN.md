...
- `projects` — לקוח/אתר: שם, `site_url`, פרטי חיבור מוצפנים, פרומפטים קבועים ספציפיים (אופציונלי per-project).
...
- `prompts` — תבניות הפרומפטים הקבועים (כתיבה / תמונה כללית).
...
### Phase 1 — Auth ושלד
- Next.js app, Supabase Auth עם allowlist ל-2 אדמינים, נתיבים מוגנים.
