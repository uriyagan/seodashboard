...
**עודכן לאחרונה:** 2026-07-22 (ערב)

> עדכון: (1) הקישורים הפנימיים עברו למנוע **מבוסס-AI** (Gemini) — קורא את הפוסט + כל היעדים באתר ומציע קישורים הקשריים עם נימוק; שרת מאמת שהעוגן קיים בפוסט. (2) **אודיט רספונסיביות מלא** — סייד-בר הפך למגירה נשלפת במובייל (המבורגר), padding רספונסיבי בכל העמודים (p-5→sm:p-8→lg:60px), טבלת פוסטים מסתירה עמודות משניות במובייל, כותרות/כפתורים עוברים לשורות, מודל הוספת אתר עם גלילה. פרוס.


> עדכון אחרון: (1) עוצב מחדש רכיב הקישורים הפנימיים למנוע המלצות מבוסס-תוכן (סורק את הפוסט ומציע לקשר ביטוי ספציפי ליעד קיים — עמוד/קטגוריית מוצר/תגית מוצר/פוסט). (2) נבנה חיבור Google (OAuth יחיד) ל-**Search Console + Analytics (GA4)** — כפתור חיבור בהגדרות, בחירת נכס SC ונכס GA4 לפרויקט, שליפת מילות מפתח מדורגות, ו**דשבורד תנועה אורגנית** (קליקים/חשיפות/מיקום מ-SC + סשנים/משתמשים אורגניים מ-GA4, 28 ימים עם השוואה לתקופה קודמת, ספרקליינים וטבלת מילות חיפוש מובילות). פרוס לפרודקשן.
